...
Repository to build pymc wheels.
...
* Update pymc submodule with version you want to build:

    * cd pymc && git pull && git checkout DESIRED_COMMIT
    * cd .. && git add pymc
    * git commit

* Check minimum numpy versions to build against in ``.travis.yml`` file.  You
  need to build against the earliest numpy that pymc is compatible with; see
